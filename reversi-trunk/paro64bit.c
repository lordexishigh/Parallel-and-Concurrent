		      ...
		      int alpha, int beta)
{
  ...
  if (depth == 0)
	...
	return alphaBeta(-1, nc, nu, depth, 1-o, alpha, beta);
      ...
      for (i=0; i<n; i++) {
	_try = alphaBeta(l[i], nc, nu, depth-1, WHITE, alpha, beta);
	if (_try > alpha)
	  /* found a better move */
	  alpha = _try;
	if (alpha >= beta)
      ...
      for (i=0; i<n; i++) {
	_try = alphaBeta(l[i], nc, nu, depth-1, BLACK, alpha, beta);
	if (_try < beta)
	  /* found a better move */
	  beta = _try;
	if (alpha >= beta)
...
#if !defined(SEQUENTIAL)
int parallelSearch (int *totalExplored, int *move, int best, int *l, int noOfMoves,BITSET64 c, BITSET64 u, int noPlies, int o, int minscore, int maxscore)
{
	for(int i=0;i<noOfMoves;i++)//loops through the number of moves and through every iteration, will spawn a child which will search for a move to perform
	{ 
		multiprocessor_wait(processorAvailable); //calls wait to check if there is a processor available to use, if there isnt it will wait until one becomes available to continue
		
		if (fork() == 0) //creates a process, which will handle the move exploring
		{ 
			//calls the alphabeta function which will go through and evaluate moves, and will return the score of the best move found. Passes in the available amount of moves, the colours, the used tiles, the moves to look ahead, the turn and the minimum and maximum score. The return of the function will be stored in the moveAB variable
			int moveAB = alphaBeta(l[i], c, u, noPlies, o, minscore, maxscore);
			mailbox_send(barrier,moveAB,i,positionsExplored); //calls the mailbox_send function which will add the results to the mailbox, passing in the mailbox, the moveAB initialized above, and the positions explored
			multiprocessor_signal(processorAvailable); //calls the signal to free up the semaphore and allow other processes to access the shared memory area
			exit(0); //terminates the child processes
		}
		else if (getpid()<0){ //checks if the process id is less than 0, which would mean an error occured 
			printf("Error, pid negative, exiting\n");
			exit(-1);
		}
	}	
	printf("parent waiting for the child process to finish\n");
	while(wait(NULL)>0); //this will force the parent to wait until there are not children active.

	int move_score, move_index, positionsExplored; //local variables that will store the move score which will be used to compare the score of the current move to the score of the best move, the move index to be able to track the move in the l array, and the positions explored which will be added every itteration to intimidate the player with the enormous moves explored from the function. The variables will be initiated below by calling the mailbox_rec function.

	for(int i=0;i<noOfMoves; i++) //loops through all the moves 
	{	
		mailbox_rec(barrier,&move_score,&move_index,&positionsExplored); //calls the mailbox_rec function which will copy the data out of the buffer and into the local variables of the parent process

		printf("...parent has received a result: move %d has a score of %d\n",move_index, move_score);
		*totalExplored +=positionsExplored; //adds the positions explored in this itteration into the total ammount of explored moves
		if (move_score>best) //checks if the move scored currently explored is bigger/better than the previous best score stored
		{ 
			best = move_score; //and if it is, sets the best variable to the current move score
			*move = l[move_index]; //and sets the move pointer to point at the current move's address
		}
	}
	return best; // returns the best score which was found among the iterations
}
#endif
		      ...
		      BITSET64 c, BITSET64 u, int noPlies, int o, int minscore, int maxscore)
{
  ...
  for (i=0; i < noOfMoves; i++)
    {
      _try = alphaBeta (l[i], c, u, noPlies, o, minscore, maxscore);
      if (_try > best)
	{
	  best = _try;
	  *move = l[i];
